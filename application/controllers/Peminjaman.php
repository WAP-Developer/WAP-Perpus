...
class Peminjaman extends CI_Controller{
  ...
  function index(){
   ...
   function hapus_keranjang($nomor){
       ...
       $this->M_perpus->delete_data($w,'transaksi'); redirect(base_url().'peminjaman/lihat_keranjang');
     }

     public function selesai_booking($where){      
      $d = $this->M_perpus->find($where, 'transaksi');    
      $isi = array(  'id_pinjam' => $this->M_perpus->kode_otomatis(), 'tanggal_input' => date('Y-m-d H:m:s'),   'id_anggota' => $where,   'tgl_pinjam' => '-',  'tgl_kembali' => '-', 'totaldenda' => '0', 'status_peminjaman' => 'Booking',  'status_pengembalian' => 'Belum Kembali'  ); 
